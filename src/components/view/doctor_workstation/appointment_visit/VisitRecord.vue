<template>
  <div>
      ...
      <el-table
        ...
        <el-table-column fixed="left" label="操作" align="center" width="80">
          <template slot-scope="scope">
            <el-button size="mini" type="success" plain @click="dialogOpen(scope.row)">回 访</el-button>
          </template>
        </el-table-column>
    ...
    <!-- 回访界面 -->
    <VisitRecordEdit :visible="dialog.visible" :row="dataGrid.row"
                     :dialogClose="dialogClose" :data-grid-load-data="dataGridLoadData"/>
  </div>
</template>

<script>
import VisitRecordEdit from './VisitRecordEdit'
...
export default {
  ...
  methods: {
    ...
    dataGridLoadData () {
      this.$loading()
      let url = '/chisAPI/visitRecord/getClinicDoctorListByCriteria'
