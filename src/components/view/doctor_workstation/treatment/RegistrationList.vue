<template>
  <div>
    ...
    <el-card
      ...
      <el-table
        ...
        <el-table-column fixed="left" label="操作" align="center" width="80">
          <template slot-scope="scope">
            <el-button size="mini" type="success" plain @click="dialogOpen(scope.row)">接 诊</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="creationDate" label="挂号时间" width="170" show-overflow-tooltip/>
        <el-table-column fixed="right" prop="sysDoctorName" label="医生姓名" width="150" show-overflow-tooltip/>
      ...
      <!-- 问诊界面 -->
    <Reception :visible="dialog.visible" :mrmMemberId="mrmMemberId"
...
export default {
  ...
  methods: {
    ...
    dataGridLoadData () {
      this.$loading()
      const url = `/chisAPI/registrationRecord/getClinicListByDoctorIdFromCache`
    ...
    dialogOpen (row) {
      this.$loading()
      const url = '/chisAPI/registrationRecord/updateToCache'
      const method = 'PUT'
      row.receptionState = true
      let params = row
