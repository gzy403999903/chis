    ...
    <el-card
      ...
      <el-form :model="queryForm" ref="queryForm" :inline="true" size="mini">
        <el-form-item label="付款方式名称" prop="name">
          <el-input v-model.trim="queryForm.name" placeholder="付款方式名称 / 助记码"/>
        </el-form-item>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="现金类型" prop="cashFlag">
              <el-radio-group v-model="editForm.cashFlag">
          ...
          <el-col :span="8">
            <el-form-item label="余额类型" prop="balanceFlag">
              <el-radio-group v-model="editForm.balanceFlag">
                <el-radio :label="true">是</el-radio>
                <el-radio :label="false">否</el-radio>
              </el-radio-group>
            </el-form-item>
          ...
          <el-col :span="8">
            <el-form-item label="优惠券类型" prop="couponFlag">
              <el-radio-group v-model="editForm.couponFlag">
                <el-radio :label="true">是</el-radio>
                <el-radio :label="false">否</el-radio>
              </el-radio-group>
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    dataGridFormatterFlag (row, column, cellValue) {
      return cellValue ? '是' : '否'
    },
    dataGridLoadData () {
      this.$loading()
      const url = `/chisAPI/paymentWay/getByCriteria`
      let params = {
    ...
    dataGridDelete (row) {
      this.$confirm('确认删除吗? 该操作不可恢复！', '提示', {
        ...
        this.$loading()
        const url = `/chisAPI/paymentWay/delete/${row.id}`
        let method = 'DELETE'
    ...
    dialogOpened () {
      ...
      if (row) {
        ...
        this.dialog.url = `/chisAPI/paymentWay/update`
        this.dialog.method = 'PUT'
      } else {
        this.dialog.url = `/chisAPI/paymentWay/save`
