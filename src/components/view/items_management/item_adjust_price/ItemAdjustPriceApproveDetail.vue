  ...
  <el-dialog
    ...
    <el-card
      ...
      <el-table
        ...
        <el-table-column prop="explains" label="调价说明" min-width="200" show-overflow-tooltip/>
      </el-table>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    loadSelfUsedRecord () {
      this.$loading()
      const url = `/chisAPI/itemAdjustPrice/getByLsh`
      let params = {
        lsh: this.row.lsh
      }
    ...
    executeAction (action) {
      this.$confirm('确认执行该操作吗?', '提示', {
        ...
        this.$loading()
        const url = `/chisAPI/itemAdjustPrice/${action}`
        let method = 'PUT'
        let params = {
          lsh: this.row.lsh
        }
