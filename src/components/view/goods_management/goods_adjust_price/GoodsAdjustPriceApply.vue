<template>
  <div>
    ...
    <!-- 新建调价单 -->
    <GoodsAdjustPriceApplyEdit :visible="dialog.visible"
                               :dialog-close="dialogClose" :data-grid-load-data="dataGridLoadData"/>
  </div>
</template>

<script>
import GoodsAdjustPriceApplyEdit from './GoodsAdjustPriceApplyEdit'
export default {
  components: {
    GoodsAdjustPriceApplyEdit
  },
  data () {
    return {
  ...
  methods: {
    ...
    dataGridLoadData () {
      this.$loading()
      const url = `/chisAPI/goodsAdjustPrice/getByCriteria`
