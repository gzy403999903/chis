  ...
  <el-dialog
    ...
    <el-row slot="title">
      <el-col :span="5" style="font-size: 20px;">
        <span>领用申请单</span>
        <span style="font-size: 13px; padding-left: 20px;">提示: 只能从退货库领用</span>
      </el-col>
    ...
    <el-card
      ...
      <el-table
        ...
        <el-table-column fixed="left" prop="quantity" label="领用数量" width="100" show-overflow-tooltip>
          <template slot-scope="props">
...
export default {
  ...
  methods: {
     ...
     */
    getSubmitData () {
      let selfUsedRecordList = []
      this.dataGrid.data.forEach(item => {
        let selfUsedRecord = {}
        selfUsedRecord.iymInventoryId = item.id
        selfUsedRecord.gsmGoodsId = item.gsmGoodsId // 商品ID
        selfUsedRecord.ph = item.ph // 批号
        selfUsedRecord.pch = item.pch // 批次号
        selfUsedRecord.splitQuantity = item.splitQuantity // 拆分系数
        selfUsedRecord.quantity = item.quantity // 出库数量
        selfUsedRecord.costPrice = item.costPrice // 成本价
        selfUsedRecord.averageCostPrice = item.averageCostPrice // 平均成本价
        selfUsedRecord.purchaseTaxRate = item.purchaseTaxRate // 进货税率
        selfUsedRecord.sellTaxRate = item.sellTaxRate // 销售税率
        selfUsedRecord.producedDate = item.producedDate // 生产日期
        selfUsedRecord.expiryDate = item.expiryDate // 有效期至
        selfUsedRecord.pemSupplierId = item.pemSupplierId // 供应商ID
        selfUsedRecord.iymInventoryAddId = item.iymInventoryAddId // 入库ID
        selfUsedRecord.notes = item.notes // 备注

        selfUsedRecordList.push(selfUsedRecord)
      })
      return selfUsedRecordList
    },
    ...
    submitData () {
      ...
      this.$confirm('确认提交吗?', '提示', {
        ...
        // 提交数据
        this.$loading()
        const url = `/chisAPI/selfUsedRecord/saveList`
        let method = 'POST'
        let data = {
          selfUsedRecordListJson: JSON.stringify(this.getSubmitData())
        }
