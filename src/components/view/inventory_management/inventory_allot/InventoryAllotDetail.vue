<template>
  <el-dialog
    ...
    <el-table
      ...
      <el-table-column prop="creationDate" label="单据日期" width="160"/>
      ...
      <el-table-column prop="ph" label="批号" width="140" show-overflow-tooltip/>
      <el-table-column prop="pch" label="批次号" show-overflow-tooltip/>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    dialogOpened () {
      if (!this.row.lsh) {
        this.$message.error('未能获取单据流水号')
        return false
      }

      this.$loading()
      const url = `/chisAPI/inventoryAllot/getByLsh`
      let params = {
        lsh: this.row.lsh
      }
    ...
    dataGridDoAction (action) {
      this.$confirm('确认执行该操作吗?', '提示', {
        ...
        this.$loading()
        const url = `/chisAPI/inventoryAllot/${action}`
        let method = 'PUT'
        let params = {
          lsh: this.row.lsh
        }
